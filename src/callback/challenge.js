...
const fetchData = (urlApi, callback) => {
    ...
    //en este metodo Almacena el nombre de la función que se ejecutará 
    ...
    xhttp.onreadystatechange = (event) => {
        ...
        if (xhttp.readyState === 4) {
            ...
            if (xhttp.status === 200) {
                //se ejecuta el callback recibiendo como argumentos un objeto, 
                //como la respuesta de la API es un texto plano, el metodo JSON.parse 
                //tranformará este texto en un objeto.
                //El atributo devuelve un DOMString que contiene la  
                //respuesta a la consulta como un texto o null si la consulta 
                //no tuvo exito o aun no ha sido completada.
                callback(null, JSON.parse(xhttp.responseText));
                //si la respuesta de la API no es exitosa se captura el error
            }
        } else {
            //se inicializa un objeto de tipo Error donde se le envian 
            //como argumentos un mensaje de error y la URL de la API
            // para conocer en dónde se produjo el error
            const error = new Error(`Error ${urlApi}`);
            //se ejecuta el callback recibiendo como argumentos el error 
            //y null debido a que no se pudo obtener el objeto
...
fetchData(`${API}/products`, (error1, data1) => {
     //se valida si existe un error, en caso de que exista se detiene
     // el proceso y se imprime el error
    if (error1) return console.error(error1);
    //se invoca nuevamente la función fetchData con el fin de
    // acceder a un objeto puntual del arreglo data1, se envia como 
    //parámetros la url de la API apuntando al atributo del primer objeto
    // de arreglo data1 y nuevamente una función anónima.
    fetchData(`${API}/products/${data1[0].id}`, (error2, data2) => {
        ...
        fetchData(`${API}/categories/${data2?.category?.id}`, (error3, data3) => {
             //se valida si existe error, en caso de que exista se detiene el proceso y 
             //se imprime el error
            if (error3) return console.error(error3);
            //Se imprime el objeto en la posición 1 del arreglo
            // de los objetos obtenidos en el metodo invocado inicialmente
            console.log(data1[0]);
